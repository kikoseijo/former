 ...
 */
class FormerBootstrap4 extends Framework implements FrameworkInterface
{
	/**
	...
	public function filterFieldClasses($classes)
	{
		...
		$classes = array_map(function ($class) {
			return Str::startsWith($class, 'col') ? $class : 'form-control-'.$class;
		}, $classes);
	...
	public function getInlineLabelClass($field)
	{
		...
		if ($field->isOfType('checkbox', 'checkboxes')) {
			$inlineClass = 'form-check form-check-'.$inlineClass;
	...
	{
		return Element::create('span', $text, $attributes)->addClass('form-text text-danger');
	}
